  ...
  return (
    <div className="space-y-6 animate-fade-in">
       ...
       <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
             ...
             {sortedEvents.map(event => {
               ...
               return (
                 <Card key={event.id} className="group hover:bg-white/5 transition-all cursor-pointer flex gap-4 items-center">
                    <div className="flex flex-col items-center justify-center min-w-[60px] h-[60px] bg-track-green border border-white/10 rounded-lg group-hover:border-track-gold transition-colors">
                    ...
                    <div className="flex-1">
                       <div className="flex justify-between items-start">
                          ...
                          <Badge color={event.type === 'Meet' ? 'bg-track-gold text-track-green' : 'bg-white/10 text-white'}>{event.type}</Badge>
                       </div>
          ...
          <Card className="hidden lg:flex flex-col justify-center items-center text-center bg-gradient-to-br from-track-surface to-track-green border-track-gold/20">
             <div className="p-4 rounded-full bg-track-gold/10 mb-4 animate-pulse">
                <Clock className="w-8 h-8 text-track-gold" />
             </div>
             <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-2">Next Major Event</h3>
             <p className="text-3xl font-black text-white font-display uppercase tracking-widest mb-6">Regional Qualifiers</p>
             <div className="flex gap-4">
               {['02', '14', '32'].map((num, i) => (
                 <div key={i} className="text-center">
                    <span className="block text-2xl font-mono font-bold text-track-gold">{num}</span>
                    <span className="text-[10px] text-gray-500 uppercase">{['Days', 'Hrs', 'Min'][i]}</span>
                 </div>
               ))}
             </div>
          </Card>
